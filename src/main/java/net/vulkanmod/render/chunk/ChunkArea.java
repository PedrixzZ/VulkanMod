    ...
    public void updateFrustum(VFrustum frustum) {
        ...
        if (frustumResult == FrustumIntersection.INTERSECT) {
            ...
            for (int x1 = 0; x1 < 2; x1++) {
                float xMin = this.position.x() + (x1 * halfWidth);
                float xMax = xMin + halfWidth;

                for (int y1 = 0; y1 < 2; y1++) {
                    float yMin = this.position.y() + (y1 * halfWidth);
                    float yMax = yMin + halfWidth;

                    for (int z1 = 0; z1 < 2; z1++) {
                        float zMin = this.position.z() + (z1 * halfWidth);
                        ...
                        if (frustumResult == FrustumIntersection.INTERSECT) {
                            ...
                            Arrays.fill(inFrustum, beginIdx, beginIdx + 8, (byte) frustumResult);
                        }
                    }
                }
            }
